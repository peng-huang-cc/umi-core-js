...
export declare class Transaction {
   ...
   * @returns {number}
   * @example
   * let timestamp = new Transaction().getTimestamp()
   */
  getTimestamp(): number
  /**
   * Устанавливает timestamp и возвращает this.
   * @param {number} timestamp Целое число в промежутке от 0 до 18446744073709551615.
   * @returns {Transaction}
   * @throws {Error}
   * @example
   * let timestamp = Date.now()/1000 | 0
   * let trx = new Transaction().setTimestamp(timestamp)
   */
  setTimestamp(timestamp: number): Transaction
  /**
   * Цифровая подпись транзакции, длина 64 байта.
   * @returns {number[]}
   * @example
